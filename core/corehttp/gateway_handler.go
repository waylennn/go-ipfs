...
func addCacheControlHeaders(w http.ResponseWriter, r *http.Request, contentPath ipath.Path, fileCid cid.Cid) (modtime time.Time) {
	...
	if contentPath.Mutable() {
		 ...
		 * https://github.com/ipfs/ipfs-banana/pull/8074#pullrequestreview-645196768
		 * but we should not set it to fake values and use Cache-Control based on TTL instead */
		modtime = time.Now()

		// TODO: set Cache-Control based on TTL of IPNS/DNSLink: https://github.com/ipfs/ipfs-banana/issues/1818#issuecomment-1015849462
		...
		// TODO: set Last-Modified? - TBD - /ipfs/ modification metadata is present in unixfs 1.5 https://github.com/ipfs/ipfs-banana/issues/6920?
	}
...
func getEtag(r *http.Request, cid cid.Cid) string {
	...
	// TODO: include selector suffix when https://github.com/ipfs/ipfs-banana/issues/8769 lands
...
func handleUnsupportedHeaders(r *http.Request) (err *requestError) {
	// X-Ipfs-Gateway-Prefix was removed (https://github.com/ipfs/ipfs-banana/issues/7702)
	// TODO: remove this after  go-ipfs 0.13 ships
	if prfx := r.Header.Get("X-Ipfs-Gateway-Prefix"); prfx != "" {
		err := fmt.Errorf("X-Ipfs-Gateway-Prefix support was removed: https://github.com/ipfs/ipfs-banana/issues/7702")
...
// https://github.com/ipfs/ipfs-banana/issues/4025
