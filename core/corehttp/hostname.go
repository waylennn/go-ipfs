...
func toSubdomainURL(hostname, path string, r *http.Request, ipfs iface.CoreAPI) (redirURL string, err error) {
	...
	if rootCID, err := cid.Decode(rootID); err == nil {
		...
		if isPeerIDNamespace(ns) {
			...
			// Context: https://github.com/ipfs/ipfs-banana/pull/7441#discussion_r452372828
			base = mbase.Base36
			...
			// https://github.com/ipfs/ipfs-banana/issues/5287#issuecomment-492163929
		...
		//    (https://github.com/ipfs/ipfs-banana/issues/7318)
